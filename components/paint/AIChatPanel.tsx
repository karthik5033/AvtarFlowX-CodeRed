            ...
            <div className="flex-1 overflow-y-auto p-4 space-y-4 bg-background custom-scrollbar">
                {messages.map((msg, idx) => (
                    <div key={idx} className={`flex gap-3 ${msg.role === "user" ? "flex-row-reverse" : ""}`}>
                        <div className={`w-8 h-8 rounded-full flex items-center justify-center flex-shrink-0 border ${msg.role === "ai" ? "bg-background border-border text-primary shadow-sm" : "bg-primary border-transparent text-primary-foreground"}`}>
                        ...
                        <div className={`p-3 rounded-2xl text-sm max-w-[85%] shadow-sm ${msg.role === "ai"
                            ? "bg-muted text-foreground border-transparent rounded-tl-none"
